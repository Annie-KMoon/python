...
def model_logistic():
    import pandas as pd
    dataset =  pd.read_csv('C:/python/05.인공지능/data/LogisticRegressionData.csv')
...
def predict_data():
    import pandas as pd
    df = pd.read_csv('C:/python/05.인공지능/data/로지스틱회귀.csv')
    ...
    return df.to_html(classes='table table-dark table-striped-columns', index=False)
